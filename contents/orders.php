...
$rslt=mysqli_query($con, "select * from orders where pendig=0 OR pendig=1");
$fp = fopen('order.csv', 'w');
$columns = 'ID,First Name,Last Name,Address 1,Address 2,City,State,Zip,Phone, Email,Username';
...
?>
  ...
  <tr>
    ...
    <input type="checkbox" onclick="status_check('<?php echo $rw['id'];?>');" name="status<?php echo     $rw['id'];?>" id="status<?php echo $rw['id'];?>" 
        ...
        <input type="button" onclick="status_check1('<?php echo $rw['id'];?>')" name="release<?php         echo $rw['id'];?>" id="release<?php echo $rw['id'];?>" value="Release Order" />
     </label>
		<?php 
		}
		else
