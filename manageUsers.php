...
//$page*2-1
  ...
  <table width="100%" class="table table-striped" border="0">
    ...
    <strong>City:</strong><?php echo html_entity_decode($cat_fetch_row['city']); ?><br />
      ...
      <strong>Company Name:</strong><?php echo html_entity_decode($row1['cname']); ?> <br /></td>
    	<td width="107"  align="left" valign="top"><?php $Date = $row1['expiration']; $orderDate = explode('-',$Date); echo $orderDate[1].'-'.$orderDate[2].'-'.$orderDate[0]; ?></td>
...
$query_Recordset1 = mysqli_query($con, "select * from userinfo WHERE `check`='2'");
...
$str1=$fname.','.$lname.','.$add1.','.$add2.','.$city.','.$st.','.$zip.','.$phn.','.$email.','.$cmp.','.$date1.','.$choice.','.$date2.','.$cer;
...
$subject = 'Usedcerticate'.@date('m-d-Y');
