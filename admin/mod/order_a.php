...
{	
	...
	$.post("mod/post2.php?id="+id,{
			...
			{
				//$("#release"+id).show();
				$("#lbl"+id).html('delivered Shipment');	
			//	change_captcha();
			//	clear_form();
			}
			else if(response==0)
...
$rslt=mysqli_query($con, "select * from orders where pendig=2");
...
?>
  ...
  <tr>
    ...
    <td><?php if($rw['pendig'] == '0' || $rw['pendig'] == '1'){?>
		...
		?>
        <input type="button" onclick="status_check2('<?php echo $rw['id'];?>')" name="send<?php echo         $rw['id'];?>" id="send<?php echo $rw['id'];?>" value="send shipment" />
        
        <?php
		             
		}?>
		<?php if($rw['pendig'] == '0' || $rw['pendig'] == '2'){?><script type="text/javascript"> $("#release<?php echo $rw['id'];?>").hide();</script><?php }?>
        <?php if($rw['pendig'] == '3'){?><script type="text/javascript"> $("#send<?php echo $rw['id'];?>").hide();</script><?php }?>
        </td>
  </tr>
  <tr>
