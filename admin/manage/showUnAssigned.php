...
$id=$_REQUEST['id'];
$pageLimit=25;
$Recordset2 = mysqli_query($con, "select * from capXlsx where status='2'");
...
goUrl('?p=showUnAssigned&mes=del');
}
if($com	==	'empty')	{
mysqli_query($con, "DELETE FROM `capXlsx` WHERE status='2'");
goUrl('?p=showUnAssigned&mes=empty');
...
<?php } ?>
<h2 class="table table-striped">Unassigned Certificates[<?php echo $totalRecords;?>]</h2>
  <div align="right"><a href="?p=showUnAssigned&com=empty" onClick= "return confirm('Do You Want To Delete All Certificates?')">Delete All Records</a></div>
  <div class="cat-heding"><a href='manage/unassicerti.php'>To Export  As Excel Click here</a></div>
    ...
    while($cat_fetch_row=mysqli_fetch_array($Recordset2)){ ?>
		<tr>
          ...
          <strong>Fulfilled: </strong><?php echo html_entity_decode($cat_fetch_row['fulfilled']); ?><br /></td>
			 <td  width="11%" valign="top"><strong>
             First Name:</strong> <?php echo html_entity_decode($cat_fetch_row['first']); ?><br />
          ...
          <strong>Choice 1: </strong> <?php echo html_entity_decode($cat_fetch_row['choice02']); ?><br />
          <strong>Choice 2: </strong> <?php echo html_entity_decode($cat_fetch_row['choice03']); ?><br />
          <strong>Choice 3: </strong> <?php echo html_entity_decode($cat_fetch_row['choice04']); ?><br />
...
// else if($exprydate >= date("m-d-Y")){
	  ...
	  echo "<div style='color:green; font-size:18px;'>Unassigned</div>";
	  }
	 
  
    ?></td>
			<!--<td><button type="button" class="remove" title="Remove this row">XDelete</button></td>-->
            <td width="9%" align="left" valign="top"><span class="btn btn-small"> <i class="icon-edit "> </i> <a href="?p=edit&pid=<?php echo $cat_fetch_row['id']; ?>">Edit</a> </span> | <span href="#" class="btn btn-small"> <i class="icon-delete "> </i> <a href="?p=showUnAssigned&com=del&page=<?php echo $page;?>&id=<?php echo $cat_fetch_row['id']; ?>" onClick= "return confirm('Do You Want To Delete This Certificate?')">Delete</a> </span></td>
		</tr>
		<?php } ?>
  </tbody>
</table>
