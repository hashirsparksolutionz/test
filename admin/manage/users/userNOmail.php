...
$id=$_REQUEST['id'];
$pageLimit=25;
$Recordset2 = mysqli_query($con, "select * from userinfo Where `email`!='' and `mailRe`='1' order by id desc;");
...
goUrl('?p=userNOmail&mes=del');
}
if($com	==	'empty'){
//mysqli_query($con, "TRUNCATE TABLE `userinfo`");
mysqli_query($con, "DELETE FROM `userinfo` WHERE `email`!=''");
goUrl('?p=userNOmail&mes=empty');
...
<?php } ?>
<h2 class="table table-striped">Users Recieved Email[<?php echo $totalRecords;?>]</h2>
  <!--<div align="right"><a href="?p=userNOmail&com=empty" onClick= "return confirm('Do You Want To Delete All Users?')">Delete All Records</a></div>-->
<div class="cat-heding"><a href='manage/users/user_emailed_xls.php'>To Export  As Excel Click here</a></div>
...
<table width="102%"  cellpadding="0" cellspacing="0" border="0"  id="table" class="tinytable">
         ...
         <thead>
                <tr>
                    ...
                    <th><h3>Certificate Demonination</h3></th>
                    <th><h3>User Demonination</h3></th>
                    <th><h3>Status</h3></th>
                    <th><h3>Action</h3></th>
          ...
          <strong>Phone:</strong><?php echo html_entity_decode($cat_fetch_row['phone']); ?><br />
           <strong>Email:</strong><?php echo html_entity_decode($cat_fetch_row['email']); ?><br />
			 ...
			 <td width="14%">
			  ...
			  $Rec= mysqli_query($con, "select * from capXlsx where certificate='".$cat_fetch_row['vocher']."'");
			  $dt=mysqli_fetch_array($Rec);
			  echo html_entity_decode($dt['choice01']); ?>
              ...
              <?php echo html_entity_decode($cat_fetch_row['Demonination']); ?>
  ...
  if($ex==1)
  {
    ...
    ?></td>
            ...
            <td width="9%" align="left"> <span href="#" class="btn btn-small"> <i class="icon-delete "> </i> <a href="?p=userNOmail&com=del&page=<?php echo $page;?>&id=<?php echo $cat_fetch_row['id']; ?>" onClick= "return confirm('Do You Want To Delete This User?')">Delete</a> </span></td>
		</tr>
		<?php } ?>
  </tbody>
</table>
