...
$id=$_REQUEST['id'];
$pageLimit=25;
$Recordset2 = mysqli_query($con, "select * from userinfo Where `mailRe`='0' order by id desc");
...
goUrl('?p=usermail&mes=del');
}
if($com	==	'empty'){
	mysqli_query($con, "DELETE FROM `userinfo` WHERE Where `email`=''");
//mysqli_query($con, "TRUNCATE TABLE `userinfo`");
goUrl('?p=usermail&mes=empty');
...
<?php } ?>
<h2 class="table table-striped">Users Not Recieved Email[<?php echo $totalRecords;?>]</h2>
  <!--<div align="right"><a href="?p=usermail&com=empty" onClick= "return confirm('Do You Want To Delete All Users?')">Delete All Records</a></div>-->
  <div class="cat-heding"><a href='manage/users/userNOtmailed_xls.php'>To Export  As Excel Click here</a></div>
...
<table width="102%"  cellpadding="0" cellspacing="0" border="0"  id="table" class="tinytable">
  ...
  if($ex==1)
  {
    ...
    ?></td>
            ...
            <td width="9%" align="left"> <span href="#" class="btn btn-small"> <i class="icon-delete "> </i> <a href="?p=usermail&com=del&page=<?php echo $page;?>&id=<?php echo $cat_fetch_row['id']; ?>" onClick= "return confirm('Do You Want To Delete This User?')">Delete</a> </span></td>
		</tr>
		<?php } ?>
  </tbody>
</table>
