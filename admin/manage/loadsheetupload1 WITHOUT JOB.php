	...
	$voucher_counts=0;
	 foreach( $xlsx->rows() as $k => $r)
	 {
		...
		if($flag==0){
			 ...
			 $q=mysqli_query($con, "SELECT * FROM `capXlsx` where status='2' AND choice01='".$r[16]."'");
...
$sql="INSERT INTO `userinfo` (`fname`,`lname`,`address1`,`city`,`state`,`zip`,`phone`,`email`,`vocher`,`beginredemption`,`expiration`,`createdOn`,`CustomerOrder`,`Exported`,`Demonination`,`InvoiceNumber`,`jobnumber`) VALUES ('".preg_replace('/[^a-zA-z0-9 ]/s',' ',$r[9])."','".preg_replace('/[^a-zA-z0-9 ]/s',' ',$r[10])."','".preg_replace('/[^a-zA-z0-9 ]/s',' ',$r[11])."','".preg_replace('/[^a-zA-z0-9 ]/s',' ',$r[12])."','".preg_replace('/[^a-zA-z0-9 ]/s','',$r[13])."','".substr(clean($r[14]),0,5)."','".replace($r[15])."','".$r[18]."','".$voucher[$row_counter]."','".$orig_date."','".$exp_date."','".$create."','".$r[7]."','".$r[8]."','".$r[16]."','".preg_replace('/[^a-zA-z0-9 ]/s',' ',$r[17])."','".$job[$row_counter]."')";
