...
{	
	...
	$.post("mod/post2.php?id_delivery="+id,{
			...
			if(response==1)
			{
				...
				$("#lb2"+id).html('Put For Delivery Shipment');
				//$("#lb2"+id).show()	;
				$("#delivery"+id).hide();
			//	change_captcha();
			//	clear_form();
...
$rslt=mysqli_query($con, "select * from orders where pendig=3 ORDER BY id DESC");
$totalRecs = mysqli_num_rows($rslt);
$fp = fopen('delivered.csv', 'w');
...
<tr>
    <td colspan="2"><a href='?p=orders_d&download_file=order.csv'>To Export  As CSV Click here</a></td>
  </tr>
<tr>
    <td style="width: 65px;"><h2><strong>Sr No</strong></h2></td>
    <td align="center"><div align="center"><h2><strong>Orders Delivered55</strong></h2></div></td>
  </tr>
<?php
$counter =1;
  ...
  <tr>
	...
	{ 
	     ?>
          ...
          <input type="button" onclick="status_delivery('<?php echo $rw['id'];?>')" name="delivery<?php echo         $rw['id'];?>" id="delivery<?php echo $rw['id'];?>" value="Put For Delivery Shipment" />
         </label>
           <script type="text/javascript"> 
...
while($order_detail_row=mysqil_fetch_assoc($order_detail)){
/////////////////////////////////////////////////////////////
...
<script type="text/javascript">
  ...
  window.open('delivered.csv','_parent');
  setTimeout(fun,1000);
  function fun(){
  window.location.href = '?p=orders_d';
  }
</script>
<?php
...
<?php }else{?><div style="margin-left: 215px;"><H2>Right Now, No New Orders Are Pending For Delivery</H2></div><?php }?>