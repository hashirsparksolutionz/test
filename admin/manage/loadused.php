...
if(isset($_POST['searchdata']) && !empty($_POST['searchdata'])){
	...
	$q = "SELECT 	c.certificate	FROM capXlsx c, userinfo u WHERE c.status='1' AND c.certificate = u.vocher AND ".$s;
	
	}else{
		$q="SELECT 	c.*,u.*	FROM capXlsx c, userinfo u where c.status='1' AND c.certificate = u.vocher ";
...
if(isset($_POST['searchdata']) && !empty($_POST['searchdata'])){
	...
	FROM capXlsx c, userinfo u WHERE c.status='1' AND c.certificate = u.vocher AND ".$s."
	...
	u.phone,
	u.size,
	u.email from capXlsx c,userinfo u where c.status='1' AND c.certificate = u.vocher  ORDER BY $field $sort limit $start,$recordsPerPage";
...
<p style="display:none" class='total_rec'><?php echo $_SESSION['countertt']; ?></p>
	...
	<?php  } ?>
<table  cellpadding="0" cellspacing="0" border="0"  id="table" class="tinytable" style="width: 100% !important;">
   ...
   <tr>
          ...
          <?php if ($cat_fetch_row['size']!='' && $cat_fetch_row['size']!=' '){  ?>

					<strong>Trip:</strong><?php echo html_entity_decode($cat_fetch_row['size']); ?><br />
	 ...
	 if($cat_fetch_row['status']==1)
  {
    ...
    ?></td>
			...
			<a href="?p=edit&pid=<?php echo $cat_fetch_row['id']; ?>"><span class="btn btn-small"> <i class="icon-edit "> </i> Edit </span></a> |
			<a href="?p=showUsedvoch&com=del&page=<?php echo $page;?>&id=<?php echo $cat_fetch_row['id']; ?>" onClick= "return confirm('Do You Want To Delete This Certificate?')"><span href="#" class="btn btn-small"> <i class="icon-delete "> </i> Delete</span></a> </td>
		</tr>
		<?php }
